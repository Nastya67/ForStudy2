    ...
    if request.args.get("find") and request.args.get("field_find"):
        ...
        for book in books:
            if str(book["book_name"]).find(find_val) != -1:
                print(str(book))
                res.append(row_to_dict_books(book))
        return str(res)
    if request.method == 'GET':
...
def row_to_dict_books(row_book):
    res = {
    "book_id": row_book[0],
    "book_name": row_book[1],
    "book_author": row_book[2],
    "book_date": row_book[3],
    "book_edition": row_book[4],
    "book_size": row_book[5],
    "book_photo": row_book[6]
    }
    return res
